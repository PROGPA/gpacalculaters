...
// Course interface with course type
interface Course {
  id: string;
  name: string;
  credits: number;
...
// Unweighted grade scale (4.0 max)
const unweightedGradeScale = {
  'A+': 4.0, 'A': 4.0, 'A-': 3.7,
  'B+': 3.3, 'B': 3.0, 'B-': 2.7,
  'C+': 2.3, 'C': 2.0, 'C-': 1.7,
...
// Weighted grade scale (5.0 max for Honors/AP)
const weightedGradeScale = {
  regular: unweightedGradeScale,
  honors: {
    'A+': 4.5, 'A': 4.5, 'A-': 4.2,
    'B+': 3.8, 'B': 3.5, 'B-': 3.2,
    'C+': 2.8, 'C': 2.5, 'C-': 2.2,
    'D+': 1.8, 'D': 1.5, 'D-': 1.2, 'F': 0.0
  },
  ap: {
    'A+': 5.0, 'A': 5.0, 'A-': 4.7,
    'B+': 4.3, 'B': 4.0, 'B-': 3.7,
...
function HighSchoolGPACalculator() {
  ...
  const [semesters, setSemesters] = useState<Course[][]>([
    [
      { id: '1', name: '', credits: 0, grade: '', gradePoints: 0, courseType: 'regular' },
      { id: '2', name: '', credits: 0, grade: '', gradePoints: 0, courseType: 'regular' },
      { id: '3', name: '', credits: 0, grade: '', gradePoints: 0, courseType: 'regular' },
      { id: '4', name: '', credits: 0, grade: '', gradePoints: 0, courseType: 'regular' }
    ]
  ]);
  ...
  const calculateGradePoints = (grade: string, courseType: 'regular' | 'honors' | 'ap', weighted: boolean) => {
    if (weighted) {
      return weightedGradeScale[courseType][grade as keyof typeof unweightedGradeScale] || 0;
    } else {
      return unweightedGradeScale[grade as keyof typeof unweightedGradeScale] || 0;
    }
  };
  ...
  const calculateGPA = useCallback(() => {
    ...
    const unweightedPoints = validCourses.reduce(
      (sum, course) => sum + (calculateGradePoints(course.grade, course.courseType, false) * course.credits), 0
    );
    
    // Calculate weighted GPA
    const weightedPoints = validCourses.reduce(
      (sum, course) => sum + (calculateGradePoints(course.grade, course.courseType, true) * course.credits), 0
    );
    ...
    const calculatedUnweightedGPA = unweightedPoints / totalCreds;
    const calculatedWeightedGPA = weightedPoints / totalCreds;
    
    setUnweightedGPA(parseFloat(calculatedUnweightedGPA.toFixed(3)));
    setWeightedGPA(parseFloat(calculatedWeightedGPA.toFixed(3)));
    setTotalCredits(totalCreds);

    // Celebration for high GPA
    const displayGPA = useWeighted ? calculatedWeightedGPA : calculatedUnweightedGPA;
    if (displayGPA >= 3.8 && validCourses.length >= 3) {
      ...
      toast.success('🎉 Outstanding High School GPA! Keep up the excellent work!');
    }
  }, [semesters, useWeighted]);
  ...
  const addSemester = () => {
    const newSemesterCourses: Course[] = [
      { id: Date.now().toString(), name: '', credits: 0, grade: '', gradePoints: 0, courseType: 'regular' },
      { id: (Date.now() + 1).toString(), name: '', credits: 0, grade: '', gradePoints: 0, courseType: 'regular' },
      { id: (Date.now() + 2).toString(), name: '', credits: 0, grade: '', gradePoints: 0, courseType: 'regular' },
      { id: (Date.now() + 3).toString(), name: '', credits: 0, grade: '', gradePoints: 0, courseType: 'regular' }
    ];
  ...
  const updateCourse = (semesterIndex: number, courseId: string, field: keyof Course, value: any) => {
    ...
    updatedSemesters[semesterIndex] = updatedSemesters[semesterIndex].map(course => {
      if (course.id === courseId) {
        ...
        if (field === 'grade' || field === 'courseType') {
          updatedCourse.gradePoints = calculateGradePoints(
            field === 'grade' ? value : course.grade,
            field === 'courseType' ? value : course.courseType,
            useWeighted
          );
        }
  ...
  const maxGPA = useWeighted ? 5.0 : 4.0;
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-lg text-gray-600 dark:text-gray-300 max-w-2xl mx-auto">
            Calculate your high school GPA including AP, Honors, and regular courses. Toggle weighted/unweighted mode to see both GPAs.
          </p>
        </div>
              ...
              <CardHeader className="bg-gradient-to-r from-purple-600 to-pink-600 text-white rounded-t-lg">
                <CardTitle className="flex items-center justify-between text-xl">
                  ...
                  <div className="flex items-center gap-3">
                    <span className="text-sm">Unweighted</span>
                    <Switch
                      checked={useWeighted}
                ...
                <CardDescription className="text-purple-100">
                  {useWeighted 
                    ? 'Weighted mode: Honors courses add 0.5 points, AP courses add 1.0 point (max 5.0 GPA)'
                    : 'Unweighted mode: All courses use standard 4.0 scale'}
                </CardDescription>
              </CardHeader>
            </Card>
                            ...
                            {semesterCourses.map((course, courseIndex) => (
                              ...
                              >
                                ...
                                <Select 
                                  ...
                                  onValueChange={(value: 'regular' | 'honors' | 'ap') => updateCourse(semesterIndex, course.id, 'courseType', value)}
                                >
                                ...
                                >
                                  ...
                                  <SelectContent>
                                    {Object.entries(unweightedGradeScale).map(([grade, points]) => (
              ...
              <CardContent className="p-6">
                <div className="space-y-6">
                  ...
                  <div className="text-center">
                    <CircularMeter value={displayGPA} maxValue={maxGPA} size={180} strokeWidth={14}>
                      <div className="text-center">
                        <div className="text-5xl font-bold mb-1" style={{ 
                          color: displayGPA >= (maxGPA * 0.925) ? '#10b981' : displayGPA >= (maxGPA * 0.75) ? '#3b82f6' : displayGPA >= (maxGPA * 0.625) ? '#f59e0b' : '#ef4444' 
                        }}>
                        ...
                        <div className="text-xs text-gray-500 dark:text-gray-400">
                          {getGPAGrade(displayGPA)}
                        </div>
                      </div>
                    </CircularMeter>
                  </div>

                  {/* Both GPAs if weighted is enabled */}
                  {useWeighted && (
                    <div className="bg-gradient-to-r from-purple-50 to-pink-50 dark:from-purple-900/20 dark:to-pink-900/20 rounded-lg p-4 border-2 border-purple-200 dark:border-purple-700">
                      <div className="flex justify-between items-center mb-2">
                        <span className="text-sm font-medium text-gray-700 dark:text-gray-300">Unweighted GPA:</span>
                        <span className="font-bold">{unweightedGPA.toFixed(2)}</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-sm font-medium text-gray-700 dark:text-gray-300">Weighted GPA:</span>
                        <span className="font-bold">{weightedGPA.toFixed(2)}</span>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="space-y-4 border-t pt-4">
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-sm font-bold">
                        {(displayGPA * totalCredits).toFixed(2)}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-4">
                    ...
                    <div className={`text-lg font-bold ${getGPAColor(displayGPA)}`}>
                      {displayGPA >= 3.7 ? 'Excellent' : 
                       displayGPA >= 3.0 ? 'Good' : 
                       displayGPA >= 2.5 ? 'Satisfactory' : 
                       displayGPA >= 2.0 ? 'Needs Improvement' : 'Poor'}
                    </div>
                  </div>
...
export { HighSchoolGPACalculator };